...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.reporteContenido = document.getElementById('reporte-contenido');
  ...
  document.querySelectorAll('.tab').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      if (panel === 'reporte') renderReporte();
    });
  });
  ...
  document.getElementById('btn-guardar-editar-grupo').addEventListener('click', () => {
    ...
    if (!nombre || !año || !facultad) return alert('Completa todos los campos');
...
function abrirGrupo(id) {
  ...
  document.getElementById('panel-reporte').classList.remove('activo');
  renderAlumnos();
}
...
function renderEvaluaciones(alumno) {
  listaEvaluaciones.innerHTML = alumno.evaluaciones.map((e, index) => `
    <div style="display:flex; justify-content:space-between; margin-bottom:4px;">
      <span>${e.nombre}</span>
      <span><strong>${e.nota}</strong></span>
...
function renderReporte() {
  ...
  const fechasAsistencia = asistenciaData[grupoActualId] 
    ? Object.keys(asistenciaData[grupoActualId]).sort() 
    : [];

  let html = `<h3>Reporte del grupo</h3>`;
  if (grupo.alumnos.length === 0) {
    html += '<p>No hay alumnos registrados.</p>';
  } else {
    grupo.alumnos.forEach(al => {
      const promedio = calcularPromedio(al.evaluaciones);
      const evaluaciones = al.evaluaciones.map(e => `${e.nombre}: ${e.nota}`).join(', ') || 'Sin evaluaciones';
      ...
      fechasAsistencia.forEach(fecha => {
        const presentes = asistenciaData[grupoActualId][fecha] || [];
        if (presentes.includes(al.id)) {
          presenteCount++;
          badges += `<span class="badge badge-presente">${fecha}</span> `;
        } else {
          badges += `<span class="badge badge-ausente">${fecha}</span> `;
        }
      });
      const totalClases = fechasAsistencia.length;
      const porcentaje = totalClases ? ((presenteCount / totalClases) * 100).toFixed(0) : 'N/A';

      html += `
        <div class="reporte-alumno">
          <h4>${al.nombre} ${al.apellido}</h4>
          <div class="reporte-detalle">
            <p><strong>Evaluaciones:</strong> ${evaluaciones}</p>
            <p><strong>Promedio:</strong> ${promedio}</p>
            <p><strong>Asistencia:</strong> ${presenteCount}/${totalClases} (${porcentaje}%)</p>
            <div class="asistencia-fechas">${badges}</div>
          </div>
        </div>
      `;
    });
